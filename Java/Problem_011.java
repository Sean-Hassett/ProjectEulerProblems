	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < gridArray.length - 3; i++){
			for (int j = 0; j < gridArray[0].length - 3; j++){
				...
				diagonalRightProduct = gridArray[i][j] * gridArray[i+1][j+1] * gridArray[i+2][j+2] * gridArray[i+3][j+3];
				diagonalLeftProduct = gridArray[i][j+3] * gridArray[i+1][j+2] * gridArray[i+2][j+1] * gridArray[i+3][j];
